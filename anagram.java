...
/**
 * Wordlist should be a variable inside and move useful constants inside as
 * constants
 * 
 * @author christine
 *
...
public class anagram extends WordList implements UsefulConstants {
	...
	static int totCandidates = 0,
			// is the shortest length of a candidate
			MinimumLength = 3;
	...
	public static void main(String[] argv) {
		...
		if (argv.length < 1 || argv.length > 3) {
			// system.err
			// error the length of input should be >=1 and <=3
			e.println("Usage: java anagram  string-to-anagram " + "[min-len [word-file]]");
			return;
		}

		if (argv.length >= 2)
			// sets the minimum length to the length of the argument passed in
			MinimumLength = Integer.parseInt(argv[1]);

		// so if argv length is 1 it will do nothing

		// word filename is optional 3rd argument

		// if argv length is 3 then pass in the second word in argu
		// if argv length is not 3 then pass in words.txt
	...
	static void getCandidates(Word d) {
		for (int i = totCandidates = 0; i < totWords; i++)
			if ((Dictionary[i].total >= MinimumLength)
					&& (Dictionary[i].total + MinimumLength <= d.total || Dictionary[i].total == d.total)
